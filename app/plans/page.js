...
export default function Plans() {
    ...
    const formatItinerary = (itinerary, itineraryId) => {
        ...
        return itinerary.map((item, index) => (
            <div key={index}>
                <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mt: 2 }}>{`${item.time} - ${item.name}`}</Typography>
                <Collapse in={expanded[itineraryId]} timeout="auto" unmountOnExit>
                    <Typography variant="body2" sx={{ mt: 1 }}>{item.description}</Typography>
                    <Typography variant="caption" color="textSecondary" sx={{ display: 'block', mt: 1 }}>{`Address: ${item.address}`}</Typography>
                </Collapse>
            </div>
        ))
    }
    ...
    return (
        <div className="plans"> {/* Wrap in plans class */}
            <Container maxWidth="lg">
                <Navigation />
                <Typography variant="h4" sx={{ textAlign: 'center', mb: 4 }}>Saved Itineraries</Typography>
                                ...
                                <CardActionArea onClick={() => handleExpandClick(itinerary.id)}>
                                    <CardContent sx={{ position: 'relative' }}>
                                        ...
                                        <IconButton 
                                            ...
                                            aria-label="expand"
                                        >
                                            {expanded[itinerary.id] ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                                        </IconButton>
